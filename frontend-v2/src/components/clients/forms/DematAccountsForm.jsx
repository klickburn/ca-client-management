...
const emptyDemat = { brokerName: '', accountNumber: '', username: '', password: '' };

export default function DematAccountsForm({ accounts = [], onChange }) {
  ...
  const add = () => onChange([...accounts, { ...emptyDemat }]);
  ...
  return (
    <div className="space-y-4">
      {accounts.map((acc, i) => (
        <div key={i} className="bg-muted rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-white">{acc.brokerName || `Demat ${i + 1}`}</span>
      ...
      <Button type="button" variant="outline" size="sm" onClick={add}>
        <Plus size={14} className="mr-1" /> Add Demat Account
      </Button>
    </div>
  );
}
